...
export default function CartFeature() {
  ...
  const [cartItems, setCartItems] = useState<CartItem[]>([
    ...
    },
    {
      ...
      quantity: 2,
      image:
        "https://images.unsplash.com/photo-1533134242443-d4fd215305ad?w=200",
      maxStock: 3,
    },
  ]);
      ...
      prev.map((item) => {
        if (item.id === id) {
          ...
          if (newQty > item.maxStock) {
            alert(
              `Sản phẩm này chỉ còn tối đa ${item.maxStock} sản phẩm trong kho.`,
            );
            return item;
          }
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 border-b border-gray-200 pb-5">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              Kiểm tra lại danh sách bánh trước khi tiến hành thanh toán.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-4">
            {cartItems.map((item) => (
              <div
                key={item.id}
                className="bg-white p-4 md:p-5 rounded-2xl border border-gray-100 shadow-sm flex gap-4 md:gap-6 items-center relative group transition-all hover:shadow-md"
              >
                {/* Ảnh sản phẩm */}
                <img
                  src={item.image}
            ...
            <Link
              to="/products"
              className="inline-flex items-center gap-2 text-sm font-semibold text-gray-500 hover:text-primary transition-colors pt-2"
            >
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm space-y-4">
              ...
              <p className="text-[11px] text-gray-400 text-center mt-2 italic">
                * Miễn phí giao hàng toàn quốc cho hóa đơn trị giá từ 500.000đ
                trở lên.
              </p>
            </div>
          </div>
